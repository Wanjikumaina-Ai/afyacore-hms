// FILE: src/app/__create/social-dev-shim/page.jsx
'use client';

// Stub — @auth/create/react not available in offline Electron build
const signIn = async () => { throw new Error('Social sign-in not available in AfyaCore'); };
    ...
    <div className="min-h-screen flex items-center justify-center font-sans bg-gray-100">
      <div className="bg-white rounded-xl p-8 w-full max-w-[400px] shadow-md">
        <div className="bg-amber-50 border border-amber-400 rounded-lg p-3 mb-4 text-[13px] text-amber-800">
          <strong>Development Mode</strong> — This is a simulated {label} sign-in.
        </div>
        ...
        {missingSecrets && missingSecrets.length > 0 && (
          <div className="bg-red-50 border border-red-400 rounded-lg p-3 mb-4 text-[13px] text-red-900">
            <strong>Missing secrets</strong> — {label} sign-in won't work until you add:{' '}
            {missingSecrets.map((s) => (
              <code key={s} className="bg-red-200 px-1 rounded text-[12px]">{s}</code>
            ))}
          </div>
        )}

        <h2 className="mt-0 mb-6 text-xl font-semibold">Sign in with {label}</h2>
